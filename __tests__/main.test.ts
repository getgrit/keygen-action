...
describe('run()', () => {
  it('resolves correctly on a happy path', async () => {
    ...
    process.env['INPUT_ARTIFACTS-JSON'] = JSON.stringify([
      {
        filepath: './__tests__/dummy_artifact.txt',
        platform: 'darwin',
        arch: 'amd64'
      }
    ])
    ...
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        status: 201,
        json: async () => ({data: {id: 'SOME_SERVER_GENERATED_RELEASE_ID'}})
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        status: 307,
    ...
    expect(fetchMock.mock.calls[0][0]).toEqual(
      `https://api.keygen.sh/v1/accounts/${process.env['INPUT_ACCOUNT-ID']}/releases`
    )
    ...
    expect(fetchMock.mock.calls[1][0]).toEqual(
      `https://api.keygen.sh/v1/accounts/${process.env['INPUT_ACCOUNT-ID']}/artifacts`
    )
    ...
    expect((fetchMock.mock.calls[2][1]?.body as ReadStream).path).toEqual(
      './__tests__/dummy_artifact.txt'
    )

    // 6.4. Release publish
    expect(fetchMock.mock.calls[3][0]).toEqual(
      `https://api.keygen.sh/v1/accounts/${process.env['INPUT_ACCOUNT-ID']}/releases/SOME_SERVER_GENERATED_RELEASE_ID/actions/publish`
    )
